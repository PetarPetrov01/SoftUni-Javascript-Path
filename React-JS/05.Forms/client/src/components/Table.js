...
export const Table = ({
    ...
    }, [sorting, onSortChange, shouldSort]);

    function onSortSelection(e) {
        e.preventDefault();
        const currentThName = e.target.closest('th').textContent;
        if (sorting[currentThName] === undefined) {
            console.log('Not a sorting el');
            return;
        }

        const newSort = { ...sorting };
        for (let key in newSort) {
            if (key !== currentThName) {
                newSort[key] = '';
            }
        }

        newSort[currentThName] = newSort[currentThName] === 'desc'
            ? 'asc'
            : 'desc';
